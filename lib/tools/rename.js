...
export function applyRenameTool(ctx) {
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            // 只改标题：低风险，直接执行，无需审批。
            if (!hasPath) {
                await ws.setTitle(args.new_title);
                return { workspace: projectWorkspace(ws), repointed: false, note: '已更新显示标题。' };
            }
            // 改路径：先做无副作用的预检（目录存在性 + 冲突），再审批，最后提交。
            let existingAtNewPath;
            try {
                existingAtNewPath = await ctx.workspaceRegistry.resolveByPath(args.new_path);
            }
            catch {
                throw new HarnessError("无法改注册路径：'" + args.new_path + "' 必须是一个已存在的目录。", 'WORKSPACE_PATH_INVALID');
            }
            if (existingAtNewPath !== undefined && String(existingAtNewPath.id) !== args.workspace_id) {
                throw new HarnessError("无法改注册路径：'" + args.new_path + "' 已被另一个工作区 '" + existingAtNewPath.title + "' 注册。", 'WORKSPACE_PATH_CONFLICT');
            }
            if (existingAtNewPath !== undefined && String(existingAtNewPath.id) === args.workspace_id) {
            ...
            const newTitle = hasTitle ? args.new_title : ws.title;
            await requireApproval(ctx, exec, 'rename_workspace', "将工作区 '" + ws.title + "'（" + ws.path + "）的注册路径重新指向 '" + args.new_path + "'；旧注册将被删除，原会话关联不会迁移。");
            // 先建新注册、成功后再删旧注册：把「先删后建」的不可恢复数据丢失风险
            // （旧注册已删、新注册因 storage 故障未建成）降级为「先建后删」的可恢复
            // 短暂重复注册（新建成功、旧删失败时短暂并存两条记录，可重试清理）。
            const repointed = await ctx.workspaceRegistry.create(args.new_path, newTitle);
